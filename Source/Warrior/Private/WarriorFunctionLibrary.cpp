...
FGameplayTag UWarriorFunctionLibrary::ComputeHitReactDirectionTag(AActor* InAttacker, AActor* InVictim, float& OutAngleDifference)
{
	...
	FVector VictimForward = InVictim->GetActorForwardVector();
	FVector VictimToAttackerNormalized = (InAttacker->GetActorLocation() - InVictim->GetActorLocation()).GetSafeNormal();
	...
	FLatentActionInfo LatentInfo)
{
	...
	FWarriorCountDownAction* FoundAction = LatentActionManager.FindExistingAction<
		FWarriorCountDownAction>(LatentInfo.CallbackTarget, LatentInfo.UUID);
	...
	case EWarriorInputMode::GameOnly:
		{
			...
			PlayerController->SetInputMode(GameModeOnly);
	...
	case EWarriorInputMode::UIOnly:
		{
			...
			PlayerController->SetInputMode(UIModeOnly);
